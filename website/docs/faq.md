...
CFA v0.1.9 targets batch and micro-batch execution. Streaming support is planned for a future version. The architecture's concurrency model assumes `single_active_intent_per_target_scope`.

## Is CFA production-ready?

CFA v0.1.9 has 343 passing tests, a full CLI, MCP server, rich reporting, and framework adapters. It is designed for production governance gates. The core has been battle-tested through multiple refactors. As with any governance tool, deploy with monitoring and start with `cfa evaluate` in CI before moving to runtime gates.
