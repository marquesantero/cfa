...
class TestAuditTrailHashChain:
    ...
    def test_hash_chain_detects_tampering(self):
        ...
        trail.record("i1", "policy", "eval", "approved")
        storage.load_all()[0].outcome = "blocked"
        assert not trail.verify_chain()

    def test_legacy_truncated_hash_still_verifies(self):
        storage = InMemoryAuditStorage()
        event = AuditEvent(intent_id="i1", stage="policy", event_type="eval", outcome="approved")
        event.event_hash = AuditTrail._compute_hash(event)[:16]
        storage.append(event)
        trail = AuditTrail(storage=storage)
        assert trail.verify_chain()
