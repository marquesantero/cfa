...
class TestKernelBlocking:
    ...
    def test_strict_normalization_blocks_unknown_dataset(self):
        kernel = KernelOrchestrator(
            catalog=CATALOG,
            config=KernelConfig(strict_normalization=True),
        )
        result = kernel.process("Processar dataset inexistente na Silver")
        assert result.state == DecisionState.BLOCKED
        assert result.policy_result is not None
        assert result.policy_result.faults[0].code == "NORMALIZATION_NO_CATALOG_DATASET_MATCH"
...
class TestKernelAuditTrail:
    def test_audit_events_populated(self):
        ...
        # Phase names are used as stage identifiers
        assert "formalize" in stages
    ...
    def test_approved_intent_has_decision_event(self):
        ...
        if result.is_executable:
            decision_events = [
                e for e in result.audit_events if e["stage"] == "validate"
            ]
