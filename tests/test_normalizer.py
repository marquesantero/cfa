...
class TestRuleBasedBackend:
    def test_grounds_known_datasets_in_catalog(self):
        normalizer = IntentNormalizer(backend=RuleBasedNormalizerBackend())
        res = normalizer.normalize(
            "Junte nfe com clientes e salve na Silver",
            environment_state={"datasets": {}},
            catalog=CATALOG,
        )
        assert {d.name for d in res.signature.datasets} == {"nfe", "clientes"}
        assert "Rule-based" in res.reasoning

    def test_marks_missing_catalog_match_as_high_ambiguity(self):
        normalizer = IntentNormalizer(backend=RuleBasedNormalizerBackend(strict=True))
        res = normalizer.normalize(
            "Processar dataset inexistente na Silver",
            environment_state={"datasets": {}},
            catalog=CATALOG,
        )
        assert res.confidence_score <= 0.30
        assert res.ambiguity_level == AmbiguityLevel.HIGH
        assert res.competing_interpretations
